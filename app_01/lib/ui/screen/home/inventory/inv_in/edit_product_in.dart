...
class EditProductInPage extends StatefulWidget {
  final index;
  final grpcInvInReqDetailModel invInReqDetailModel;
  final List<grpcSelectProductModel> productSlistData;

  const EditProductInPage(
      {Key? key,
      this.index,
      required this.invInReqDetailModel,
      required this.productSlistData})
      : super(key: key);

  @override
  _EditProductInPageState createState() => _EditProductInPageState();
}

class _EditProductInPageState extends State<EditProductInPage> {
  ...
  void initState() {
    ...
    _masterBloc.add(GetProductRecordInit(
        productCode: widget.invInReqDetailModel.productCode));
    _cntProduct.dropDownValue = DropDownValueModel(
        name: widget.invInReqDetailModel.productName,
        value: widget.invInReqDetailModel.productCode);
    _cntUnit.dropDownValue = DropDownValueModel(
        name: widget.invInReqDetailModel.unitName,
        value: widget.invInReqDetailModel.unitCode);
    _invInReqDetailModel.productName = widget.invInReqDetailModel.productName;
    _etSpecification.text = widget.invInReqDetailModel.specification;
    _etPackingQty.text = widget.invInReqDetailModel.packingQty.units.toString();
    _etCaseQty.text = widget.invInReqDetailModel.caseQty.units.toString();
    _etReqQty.text = widget.invInReqDetailModel.reqQty.units.toString();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: _globalWidget.globalAppBar(Text('Chỉnh sửa sản phẩm nhập kho',
            style: TextStyle(
              ...
              BlocListener<MasterBloc, MasterState>(
                listener: (context, state) {
                  if (state is GetProductRecordInitSuccess) {
                    ...
                    // setState(() {
                    //   _valScrollUnit = widget.invInReqDetailModel.unitCode;
                    // });
                    setState(() {
                  ...
                  if (state is GetProductRecordSuccess) {
                    ...
                    _invInReqDetailModel.productName =
              ...
              _globalWidget.createButton(
                  ...
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      copyPropertiesData();
                      _inventoryCubit.editProductIn(_invInReqDetailModel);
                    }
                  }),
            ],
          ),
  ...
  void copyPropertiesData() {
    _invInReqDetailModel.lineNo = widget.invInReqDetailModel.lineNo;
