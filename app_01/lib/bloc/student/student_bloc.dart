...
void _getStudent(GetStudent event, Emitter<StudentState> emit) async {
  ...
  try {
    List<grpcStockSumModel> data = await _apiProvider.getStudent("", "");
...
//     try {
//       List data = await _apiProvider.addStudent(event.sessionId, event.studentName, event.studentPhoneNumber, event.studentGender, event.studentAddress, event.apiToken);
...
//     try {
//       String message = await _apiProvider.editStudent(event.sessionId, event.studentId, event.studentName, event.studentPhoneNumber, event.studentGender, event.studentAddress, event.apiToken);
...
//   try {
//     String msg = await _apiProvider.deleteStudent(event.sessionId, event.studentId, event.apiToken);
